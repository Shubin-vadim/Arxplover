...
class MultiModalWeaviateRetriever:
    ...
    def add_documents(self, texts:list[str], metadatas: list[dict] | None = None) -> None:
        ...
        logger.info(f"Adding {len(texts)} text documents to the vector store.")
        try:
            ...
            logger.error(f"Error loading excel data: {error}")
            raise error
    ...
    def retrieve(self, query: str, top_k: int = 3) -> list[Document]:
        ...
        results = list(self.vector_store.similarity_search(query, top_k))
